...
async function createBookings() {
    ...
    const { data: guestsIds, error: guestsError } = await supabase.from("guests").select("id").order("id");
    ...
    const { data: cabinsIds, error: cabinsError } = await supabase.from("cabins").select("id").order("id");
    ...
    const finalBookings = bookings.map((booking) => {
        ...
        if (!cabin) {
            console.log(`Cabin not found for booking ID: ${booking.cabinId}`);
